...
fit_model_final <- function(time_iters,
                      n_months_fit,
                      response_mat,
                      A,
                      spde,
  ...
  for(time_iter in time_iters){
    ...
    if(is.null(subset_use)){
      N_covs_total <- dim(cov_mat_list[[1]])[2]
      subset_use_iter <- 1:N_covs_total
    }else{
      subset_use_iter <- subset_use[[time_iter]]
    }
    
    N_covs <- length(subset_use_iter)
    print(subset_use_iter)
    # print(N_covs)
    # 
    # print(subset_use)
    # print(dim(cov_mats_fit[[1]]))
    cov_mat_list_use <- lapply(cov_mat_list, function(mat) mat[, subset_use_iter, drop=FALSE])
    
    
    print(paste0("time iter: ", time_iter))
    time_index_fit <- (time_iter - 1) + 1:n_months_fit
    cov_mat_fit <- cov_mat_list_use[time_index_fit]
    # print(time_index_fit)
    outbreaks_fit <- response_mat[, time_index_fit]
    # plot(cov_mat_fit[[1]][1, ], main=time_iter)
