...
fit_model_mesh <- function(time_iters,
  ...
  # for(time_iter in 1:1){
    ...
    #make projection mat list
    n_mesh_points <- length(index_keep_cpp)
    ...
    for(i in 1:n_months_fit){
      K_proj_a <- matrix(NA, nrow=n_locs, ncol=n_mesh_points)
      cov_mat_t <- cov_mat_fit[[i]]
      for(j in 1:n_locs){
        for(k in 1:n_mesh_points){
          K_proj_a[j, k] <- exp(-sum((cov_mat_t[j, ] - cov_mat_mesh[k, ])^2)/sigma)
        }
      }
      
      K_proj <- K_proj_a %*% K_inv
      
      proj_mat_list[[i]] <- K_proj
    }
