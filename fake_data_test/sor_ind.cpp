...
template <class Type> 
  struct LOLOM_t : vector<LOM_t<Type> >{
    LOLOM_t(SEXP x){ /*x is list from R*/
      (*this).resize(LENGTH(x)); /*resize pointer to be the right size*/
      for(int i=0; i<LENGTH(x); i++){ 
        SEXP m = VECTOR_ELT(x, i); /*get ith element of list */
        (*this)(i) = m; /*put it as ith element*/
      }
    }  
  };
  ...
  Type objective_function<Type>::operator() ()
{
  ...
  for(int i=0; i<n_t; i++){
    vector<Type> vals_all = proj_mats(i) * vals;
    for(int j=0; j<n; j++){
      f -= dpois(Y(j, i), exp(field(j) + beta0 + vals_all(j)) * pops(i), true);
    }
  }
  // for(int i=0; i<n; i++){
