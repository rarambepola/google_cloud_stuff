...
load(paste0("covariates_all.RData"))
...
tryCatch(dyn.unload(dynlib("outbreaks_lasso")),
         error = function(e) print(e))
compile("outbreaks_lasso.cpp")
dyn.load(dynlib("outbreaks_lasso"))
...
for(beta_i in 5:2){
  fit_rep_list <- fit_model_final(time_iters,
                                  n_months_fit,
                                  outbreaks_fit_full,
                                  A_fit,
                                  spde,
                                  ...
                                  eval.max=500,
                                  use_lasso=TRUE,
                                  beta_scale = beta_scales[beta_i]
                                  )

  fit_rep_list_list[[beta_i]] <- fit_rep_list
  
  save(list = c("fit_rep_list", "fit_rep_list_list", "beta_scales"),
       file = paste0("lasso_step_one_beta_", beta_i, ".RData"))
  # 
  # tryCatch(save(list = c("fit_rep_list", "fit_rep_list_list", "beta_scales"),
  #               file = paste0(zdrive_path, "rohan/lasso_step_one_beta_", beta_i, ".RData")),
  #          error = function(e) print(e))
}
...
# save(list = c("fit_rep_list_list"),
#      file = paste0("../../v4_MDG_big_files/lasso_step_one"))
