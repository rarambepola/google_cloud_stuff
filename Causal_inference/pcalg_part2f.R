...
pcalg.second <- function(obsDat, alpha, test.index, G_0 = NULL, supprMessages = FALSE){
  nvar <- length(obsDat)
  ...
  skeleton <- pcskel.last(G_0, obsDat, alpha, test.index, supprMessages)
...
pcskel.last <- function(G_0, obsDat, alpha, test.index, supprMessages = FALSE){
  ...
  getEdgesLast <- function(adjMat, test.index){
    n <- dim(adjMat)[1]
    edges <- list()
    for(i in test.index){
      for(j in 1:n){
...
edges <- getEdgesLast(G_0, test.index)
...
ptm <- proc.time()
edges <- getEdgesLast(G_old, test.index)
maxsize <- 0
