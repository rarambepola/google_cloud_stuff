...
forward_pred_vec <- function(covs, fit, rep, A){
...
forward_pred <- function(fit, covs, rep, A, as_matrix=TRUE){
  if(as_matrix){
    return(do.call("cbind", lapply(covs, forward_pred_vec, fit, rep, A)))
  }else{
    return(lapply(covs, forward_pred_1, fit, rep, A))
  }
}
